...
public class QdrantService : IQdrantService
{
    ...
    public QdrantService(string endpoint, int vectorSize, ILoggerFactory? loggerFactory = null)
    {
        this._qdrantRepository = new QdrantRepository(endpoint, vectorSize, loggerFactory);
        this._logger = loggerFactory?.CreateLogger(typeof(QdrantService)) ?? NullLogger.Instance;
    }
    ...
    /// <param name="loggerFactory">The <see cref="ILoggerFactory"/> to use for logging. If null, no logging will be performed.</param>
    public QdrantService(HttpClient httpClient, int vectorSize, string? endpoint = null, ILoggerFactory? loggerFactory = null)
    {
        this._qdrantRepository = new QdrantRepository(httpClient, vectorSize, endpoint, loggerFactory);
        this._logger = loggerFactory?.CreateLogger(typeof(QdrantService)) ?? NullLogger.Instance;
    }
    ...
    /// Initializes a new instance of the <see cref="QdrantMemoryStore"/> class.
    /// </summary>
    /// <param name="qdrantRepository">The Qdrant Db client for interacting with Qdrant Vector Database.</param>
    /// <param name="loggerFactory">The <see cref="ILoggerFactory"/> to use for logging. If null, no logging will be performed.</param>
    public QdrantService(IQdrantRepository qdrantRepository, ILoggerFactory? loggerFactory = null)
    {
        this._qdrantRepository = qdrantRepository;
        this._logger = loggerFactory?.CreateLogger(typeof(QdrantService)) ?? NullLogger.Instance;
    }
    ...
    {
        if (!await this._qdrantRepository.DoesCollectionExistAsync(collectionName, cancellationToken).ConfigureAwait(false))
        {
            await this._qdrantRepository.CreateCollectionAsync(collectionName, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    {
        return await this._qdrantRepository.DoesCollectionExistAsync(collectionName, cancellationToken).ConfigureAwait(false);
    }
        public async Task<string> UpsertAsync(string collectionName, MemoryRecord record, CancellationToken cancellationToken = default)
    {
        var vectorData = await this.ConvertFromMemoryRecordAsync(collectionName, record, cancellationToken).ConfigureAwait(false) ??
            throw new Exception("Failed to convert memory record to Qdrant vector record");

        try
        {
            await this._qdrantRepository.UpsertVectorsAsync(
                collectionName,
                [vectorData],
    ...
    {
        return this._qdrantRepository.ListCollectionsAsync(cancellationToken);
    }
    ...
    public async Task DeleteCollectionAsync(string collectionName, CancellationToken cancellationToken = default)
    {
        if (await this._qdrantRepository.DoesCollectionExistAsync(collectionName, cancellationToken).ConfigureAwait(false))
        {
            await this._qdrantRepository.DeleteCollectionAsync(collectionName, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    public async Task<MemoryRecord?> GetAsync(string collectionName, string key, bool withEmbedding = false, CancellationToken cancellationToken = default)
    {
        try
        {
            var vectorData = await this._qdrantRepository.GetVectorByPayloadIdAsync(collectionName, key, withEmbedding, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var vectorDataList = this._qdrantRepository
    ...
    public async Task RemoveAsync(string collectionName, string key, CancellationToken cancellationToken = default)
    {
        try
        {
            await this._qdrantRepository.DeleteVectorByPayloadIdAsync(collectionName, key, cancellationToken).ConfigureAwait(false);
        }
        catch (HttpOperationException ex)
        {
    ...
    public async Task RemoveWithPointIdAsync(string collectionName, string pointId, CancellationToken cancellationToken = default)
    {
        try
        {
            await this._qdrantRepository.DeleteVectorsByIdAsync(collectionName, [pointId], cancellationToken).ConfigureAwait(false);
        }
        catch (HttpOperationException ex)
        {
            this._logger.LogError(ex, "Failed to remove vector data: {Message}", ex.Message);
    ...
    public async Task RemoveWithPointIdBatchAsync(string collectionName, IEnumerable<string> pointIds, CancellationToken cancellationToken = default)
    {
        try
        {
            await this._qdrantRepository.DeleteVectorsByIdAsync(collectionName, pointIds, cancellationToken).ConfigureAwait(false);
        }
        catch (HttpOperationException ex)
        {
            this._logger.LogError(ex, "Failed to remove vector data: {Message}", ex.Message);
    ...
    public async IAsyncEnumerable<(MemoryRecord, double)> GetNearestMatchesAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        IAsyncEnumerator<(QdrantVectorRecord, double)> enumerator = this._qdrantRepository
        ...
        do
        {
            ...
            catch (HttpOperationException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                this._logger.LogWarning("NotFound when calling {QdrantMemoryStore}::FindNearestInCollectionAsync - the collection '{Name}' may not exist yet",
                    nameof(QdrantService), collectionName);
                hasResult = false;
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var existingRecord = await this._qdrantRepository.GetVectorByPayloadIdAsync(
                    collectionName,
                    record.Metadata.Id,
                    cancellationToken: cancellationToken)
            ...
            else
            {
                do // Generate a new ID until a unique one is found (more than one pass should be exceedingly rare)
                {
                    ...
                    existingRecord = await this._qdrantRepository.GetVectorsByIdAsync(collectionName, [pointId], cancellationToken: cancellationToken)
